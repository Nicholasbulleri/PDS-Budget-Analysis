...
-- EXCLUDES projects with missing or zero area: only rows where
-- COALESCE(NULLIF(grossarea, 0), usablearea) IS NOT NULL AND > 0.
-- Otherwise identical to budget_core_query_closed_by_category.sql.
-- Output: taskname = category, city, country, sector, area, total_original_budget,
--         total_projected_budget, project_count.
-- State is applied externally via mapping (see run_..._exclude_missing_area.py).
