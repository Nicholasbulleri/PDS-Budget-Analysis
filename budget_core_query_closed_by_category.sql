-- Budget Closed query grouped by 3 categories (FF&E + Millwork, Soft Costs, Construction).
-- Uncategorized task names are excluded; their budget amounts are not included.
-- Output: taskname = category (3 values), city, country, sector, area, total_original_budget,
--         total_projected_budget, project_count. Schema matches original query; result set is much smaller.
-- Source: work_dynamics.curated (generictask + project), Closed USD, budget + Cost Code Item.
-- Mapping logic is inlined in the CASE (same intent as budget_taskname_category_mapping).
    ...
    SELECT
        *,
        CASE
            ...
            -- CSI division (01 - through 32 -, or leading dash - NN -)
