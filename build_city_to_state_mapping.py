...
1. Reads unique (city, country) from budget_query_results_curated_closed_by_category_with_state.csv
...
3. Writes city_to_state_mapping.csv (city, country, state_abbrev) for use in the SQL query.

Run once to generate the mapping; then update the query to use it (e.g. join or inline table).
"""
...
BUDGET_CSV = os.path.join(SCRIPT_DIR, "budget_query_results_curated_closed_by_category_with_state.csv")
OUT_CSV = os.path.join(SCRIPT_DIR, "city_to_state_mapping.csv")
REFERENCE_URL = "https://raw.githubusercontent.com/grammakov/USA-cities-and-states/master/us_cities_states_counties.csv"
REFERENCE_LOCAL = os.path.join(SCRIPT_DIR, "us_cities_states_counties.csv")
        ...
        if not text:
            try:
                ...
                try:
                    ...
                    raise SystemExit(
                        f"Could not load reference. Save us_cities_states_counties.csv to {SCRIPT_DIR}.\nURL error: {e1}; fallback: {e2}"
                    ) from e2
    if not text:
...
def main() -> int:
    if not os.path.isfile(BUDGET_CSV):
        print(f"Budget CSV not found: {BUDGET_CSV}")
        return 1
