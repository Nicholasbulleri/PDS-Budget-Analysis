#!/usr/bin/env python3
"""
Run Ingenious projects area/city/country/property analysis.
Scope: phase Closed and In Progress only. Results segmented by phase.
Executes the 3 queries from ingenious_projects_area_city_country_property_analysis.sql.
"""
import os
import csv
...
OUT_CSV = os.path.join(PROJECT_ROOT, "data", "analysis", "ingenious_projects_area_city_country_property_analysis.csv")
OUT_REPORT = os.path.join(DATA_DIR, "ingenious_projects_area_city_country_property_analysis_report.md")
...
def main():
    ...
    WITH base AS (
      ...
      WHERE LOWER(TRIM(COALESCE(p.sourcesystem, ''))) = 'ingenious'
        {phase_filter}
    ),
    with_flags AS (
      SELECT
    ...
    report.append("# Ingenious projects: area, city, country, property analysis")
    report.append("")
    report.append("**Scope:** Phase = Closed or In Progress only. Results by phase.")
    report.append("")
    report.append("## 1. Overall counts by phase")
    report.append("")
    report.append("| Phase | Total projects | Missing area | Missing city | Missing country | Missing property |")
    report.append("|-------|----------------|--------------|--------------|-----------------|-------------------|")
    total_all = 0
    miss_area_all = miss_city_all = miss_country_all = miss_prop_all = 0
    for row in rows1:
        phase, tot, ma, mc, mco, mp = row[0] or "", int(row[1] or 0), int(row[2] or 0), int(row[3] or 0), int(row[4] or 0), int(row[5] or 0)
        total_all += tot
        miss_area_all += ma
        miss_city_all += mc
        miss_country_all += mco
        miss_prop_all += mp
        report.append(f"| {phase} | {tot:,} | {ma:,} | {mc:,} | {mco:,} | {mp:,} |")
        csv_rows.append({"section": "overall", "phase": phase, "total_projects": tot, "missing_area": ma, "missing_city": mc, "missing_country": mco, "missing_property": mp})
    report.append(f"| **Combined** | **{total_all:,}** | **{miss_area_all:,}** | **{miss_city_all:,}** | **{miss_country_all:,}** | **{miss_prop_all:,}** |")
    report.append("")
    ...
    report.append("")
    report.append("When property is **missing** vs **assigned**, by phase: total projects and counts still missing area, city, country.")
    report.append("")
    report.append("| Phase | Property status | Total projects | Missing area | Missing city | Missing country |")
    report.append("|-------|-----------------|----------------|--------------|--------------|-----------------|")
    for row in rows2 or []:
        phase, status, tot, ma, mc, mco = row[0] or "", row[1], int(row[2] or 0), int(row[3] or 0), int(row[4] or 0), int(row[5] or 0)
        report.append(f"| {phase} | {status} | {tot:,} | {ma:,} | {mc:,} | {mco:,} |")
        csv_rows.append({"section": "by_property", "phase": phase, "property_status": status, "total_projects": tot, "missing_area": ma, "missing_city": mc, "missing_country": mco})
    report.append("")
    report.append("**Takeaway:** \"Property assigned\" rows show how often area/city/country are still missing even when a property is linked (by phase).")
    report.append("")
    ...
    for row in rows3 or []:
        ...
        csv_rows.append({"section": "overlap", "phase": phase, "metric": metric, "when_property_missing": w_miss, "when_property_assigned": w_ass})
    report.append("")
    ...
    if csv_rows:
        keys = ["section", "phase", "metric", "count", "pct", "property_status", "total_projects", "missing_area", "missing_city", "missing_country", "missing_property", "when_property_missing", "when_property_assigned"]
