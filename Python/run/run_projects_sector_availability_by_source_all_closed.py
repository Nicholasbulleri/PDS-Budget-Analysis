#!/usr/bin/env python3
"""
Projects with/without sector by source system (Clarizen, Ingenious) — Option A: all Closed.
Population: All projects in phase Closed only (no USD or budget requirement).
...
SQL_FILE = os.path.join(SQL_DIR, "projects_sector_availability_by_source_all_closed.sql")
OUT_CSV = os.path.join(PROJECT_ROOT, "data", "projects", "projects_sector_availability_by_source_all_closed.csv")
OUT_REPORT = os.path.join(DATA_DIR, "projects_sector_availability_by_source_all_closed_report.md")
...
def main():
    ...
    print("Running projects sector availability by source (all Closed, no USD/budget filter)...")
    ...
    lines = [
        "# Projects with/without sector by source system (all Closed)",
        "",
        "**Population:** All projects in phase Closed only (no USD or budget requirement).",
