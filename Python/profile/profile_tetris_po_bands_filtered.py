...
"""
Tetris PO Band Analysis - Filtered to executed projects only
(Likelihood = 'extra hot' OR Project_stage = 'Finalisation')
"""
import sys, os
...
FILTER = "(p.Likelihood = 'extra hot' OR p.Project_stage = 'Finalisation')"
...
WHERE p.udp_delete_flag = 'N' AND {FILTER}
"""
...
ORDER BY po_band
""", "1. PO Band Summary (Executed Projects Only)")


# ─── 2. Asset Class (Sector) by PO Band ───
run(f"""
SELECT po_band, Asset_Class, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area
FROM (
  SELECT po_band, Asset_Class, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area,
...
run(f"""
SELECT po_band, Industry, project_count, pct_of_band, avg_total_po_amt, avg_area
FROM (
  SELECT po_band, Industry, project_count, pct_of_band, avg_total_po_amt, avg_area,
    ROW_NUMBER() OVER (PARTITION BY po_band ORDER BY project_count DESC) AS rn
  FROM (
...
run(f"""
SELECT po_band, Country, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area
FROM (
  SELECT po_band, Country, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area,
    ROW_NUMBER() OVER (PARTITION BY po_band ORDER BY project_count DESC) AS rn
  FROM (
...
# ─── 5. City by PO Band ───
run(f"""
SELECT po_band, City, project_count, pct_of_band, avg_total_po_amt, avg_area
FROM (
  SELECT po_band, City, project_count, pct_of_band, avg_total_po_amt, avg_area,
    ROW_NUMBER() OVER (PARTITION BY po_band ORDER BY project_count DESC) AS rn
  FROM (
    ...
    LEFT JOIN (
      SELECT TS_Project_ID, COUNT(*) AS po_count,
        SUM(CAST(REGEXP_REPLACE(PO_Amount, '[^0-9.-]', '') AS DOUBLE)) AS total_po_amt
      FROM {FQ}.tetris_purchaseorder WHERE udp_delete_flag = 'N' GROUP BY TS_Project_ID
    ) po ON p.Project_TS_ID = po.TS_Project_ID
...
# ─── 6. Service Type by PO Band ───
run(f"""
SELECT po_band, tetris_Service, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area
FROM (
  SELECT po_band, tetris_Service, project_count, pct_of_band, avg_po_count, avg_total_po_amt, avg_area,
    ROW_NUMBER() OVER (PARTITION BY po_band ORDER BY project_count DESC) AS rn
  FROM (
...
# ─── 7. Top 5 Supplier Categories per PO Band ───
run(f"""
SELECT po_band, supplier_category, cat_rank, po_count, project_count, total_po_amt, avg_po_amt
FROM (
  SELECT po_band, supplier_category, po_count, project_count, total_po_amt, avg_po_amt,
  ...
  FROM (
    ...
    INNER JOIN {FQ}.tetris_project p ON po.TS_Project_ID = p.Project_TS_ID
    INNER JOIN (
      SELECT TS_Project_ID,
        SUM(CAST(REGEXP_REPLACE(PO_Amount, '[^0-9.-]', '') AS DOUBLE)) AS total_po_amt
      FROM {FQ}.tetris_purchaseorder WHERE udp_delete_flag = 'N' GROUP BY TS_Project_ID
    ) proj_po ON po.TS_Project_ID = proj_po.TS_Project_ID
    LEFT JOIN {FQ}.tetris_supplier s ON po.Supplier_TS_ID = s.sup_id
    WHERE po.udp_delete_flag = 'N' AND p.udp_delete_flag = 'N' AND {FILTER}
    GROUP BY 1, COALESCE(s.Category, '(unknown)')
  ) agg
) ranked WHERE cat_rank <= 5
ORDER BY po_band, cat_rank
""", "7. Top 5 Supplier Categories per PO Band")


print("\n\n✓ Filtered PO Band analysis complete!")
