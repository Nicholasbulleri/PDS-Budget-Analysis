#!/usr/bin/env python3
"""
Run budget Closed by category (EXCLUDING missing/zero area) WITH property-derived sector for Clarizen;
apply city->state mapping OUTSIDE Databricks.
- Uses budget_core_query_closed_by_category_exclude_missing_area_with_property_sector.sql.
...
SQL_FILE = os.path.join(SCRIPT_DIR, "budget_core_query_closed_by_category_exclude_missing_area_with_property_sector.sql")
OUT_CSV = os.path.join(SCRIPT_DIR, "budget_query_results_curated_closed_by_category_with_state_using_mapping_excludes_missing_area_with_property_sector.csv")
OUT_XLSX = os.path.join(SCRIPT_DIR, "budget_query_results_curated_closed_by_category_with_state_using_mapping_excludes_missing_area_with_property_sector.xlsx")
SECTOR_MAPPING_FILE = os.path.join(SCRIPT_DIR, "sector_mapping.json")

# Canonical sector order when building lookup (first match wins; Special Purpose last as fallback)
...
def load_sector_mapping():
    """Load sector_mapping.json and return a flat lookup: normalized raw sector -> canonical sector."""
...
def main():
    ...
    query = get_query()
    print("Running budget Closed by category (excl. missing area, property sector for Clarizen)...")
    ...
    for row in results:
        ...
        # Map sector to canonical (Office, Retail, Industrial and Logistics, etc.)
    ...
    try:
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "Closed + property sector"[:31]
        ...
        import pandas as pd
        pd.DataFrame(out_rows, columns=out_columns).to_excel(OUT_XLSX, index=False, sheet_name="Closed + property sector"[:31])
